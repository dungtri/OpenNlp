...
namespace Trainer
{
    class Program
    {
        private static readonly string CurrentDirectory = Path.Combine(Environment.CurrentDirectory, "..", "..", "..") + Path.DirectorySeparatorChar;
        ...
        {
            // all directories in Input folder
            var inputFolderPath = CurrentDirectory + "Input" + Path.DirectorySeparatorChar;
            var allDirectories = Directory.GetDirectories(inputFolderPath);
            Console.WriteLine("Pick the model to train:");
            for (var i = 0; i < allDirectories.Length; i++)
            {
