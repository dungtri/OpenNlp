...
namespace OpenNLP.Tools.Coreference.Similarity
{
    ...
    public class Context : Mention.Mention
	{
        ...
        public string WordnetSearchDirectory => Path.Combine(
	        AppDomain.CurrentDomain.BaseDirectory,
	        string.Join(Path.DirectorySeparatorChar.ToString(), Enumerable.Repeat("..", 4)),
	        string.Join(Path.DirectorySeparatorChar.ToString(), "Resources", "WordNet", "dict", Path.DirectorySeparatorChar)
        );
		...
		private static Util.Set<string> GetSynsetSet(Context context)
		{
            ...
            Mention.IDictionary dictionary = Mention.DictionaryFactory.GetDictionary(context.WordnetSearchDirectory);
		...
		private static string[] GetLemmas(Context context)
		{
            ...
            return Mention.DictionaryFactory.GetDictionary(context.WordnetSearchDirectory).GetLemmas(word, PartsOfSpeech.NounSingularOrMass);
		}
	}
}